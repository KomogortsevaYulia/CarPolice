...
namespace CarPolice.Views
{
    public class TechnicalInspectionsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "conclusion,Date")] TechnicalInspection technicalInspection)
        {
            if (ModelState.IsValid)
            {
                var lastId = (from m in db.TechnicalInspection select m.id).ToList();
                if (lastId.Count==0) {
                    technicalInspection.id = 0;
                }
                else
                {
                    technicalInspection.id = lastId.Last()+1;
                }
        ...
        public ActionResult Edit([Bind(Include = "id,conclusion,Date")] TechnicalInspection technicalInspection)
        {
            if (ModelState.IsValid)
            {
                db.Entry(technicalInspection).State = System.Data.Entity.EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(technicalInspection);
